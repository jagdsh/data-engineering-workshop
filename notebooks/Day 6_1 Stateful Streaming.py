...
def unpack_json_from_status_notification_request(input_df: DataFrame) -> DataFrame:
    ...
    return input_df.withColumn("new_body", from_json(input_df.body, body_schema))
    ###
...
def aggregate_window_watermark(input_df: DataFrame) -> DataFrame:
    ...
    return input_df \
        ...
        .groupBy(window("timestamp", "5 minute"), input_df.charge_point_id, input_df.status) \
        .count() \
        # .select(col("charge_point_id"), col("status"), col("window").alias("timestamp"), )
    ###
