...
filepath = "/FileStore/jagdshlk/last_connection_time_of_charge_point/1679387766.csv"
...
def convert_to_timestamp(input_df: DataFrame) -> DataFrame:
    input_df = input_df.withColumn("converted_timestamp", to_timestamp(col("write_timestamp")))
...
lastChargePoint = Window.partitionBy('charge_point_id').orderBy(col('converted_timestamp').desc())

def most_recent_message_of_charge_point(input_df: DataFrame) -> DataFrame:
    input_df = input_df.select('message_id', 'message_type', 'charge_point_id', 'action', 'write_timestamp', 'body', 'converted_timestamp'
        ).withColumn('rn', row_number().over(lastChargePoint)
        ).filter(col("rn") <= 1) 
    return input_df
    ###
