...
df_data = {'partition': ['a','a', 'a', 'a', 'b', 'b', 'b', 'c', 'c',],
           ...
           'ranking': [4,3,2,1,1,1,2,1,5,],
           'lagging': [9,8,20,6,5,4,3,2,1,],
...
aggregation_window = Window.partitionBy('partition').orderBy(
  'aggregation',)
...
lag_window = Window.partitionBy('partition').orderBy(fn.col('lagging').desc())
...
df_lagged = df.select(
  ...
  # CAST TO Int so that we can default to "0"
  ...
  'difference_between', fn.col('lagging') - fn.col('lagging_lag_1')
)
