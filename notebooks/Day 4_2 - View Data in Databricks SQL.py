...
def drop_table(db: str, table: str):
    statement = f"DROP TABLE IF EXISTS {db}.{table};"
    print(f"Executing: {statement}")
    spark.sql(statement)
...
### YOUR CODE HERE
file_path = helpers.download_to_local_dir(urls["stop_transaction_request"])
###
...
# drop_table(
#     db=db, 
#     table="stop_transaction_request" # Your Table Name 
# )
create_table(
    db=db, 
    table="stop_transaction_request", # Your Table Name 
    data_location=file_path # The file path
)
###
