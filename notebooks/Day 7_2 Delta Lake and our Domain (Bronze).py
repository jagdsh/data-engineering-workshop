...
def set_partitioning_cols(input_df: DataFrame):
    ### YOUR CODE HERE
    return input_df \
        .withColumn("year", year(to_timestamp(col("write_timestamp")))) \
            .withColumn("month", month(to_timestamp(col("write_timestamp")))) \
                .withColumn("day", dayofmonth(to_timestamp(col("write_timestamp")))) \
                    .withColumn("hour", hour(to_timestamp(col("write_timestamp")))) \
                        .withColumn("minute", minute(to_timestamp(col("write_timestamp"))))
    ###
...
display(dbutils.fs.ls(f"{out_dir}/_delta_log"))
